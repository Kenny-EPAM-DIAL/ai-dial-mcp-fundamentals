...
class MCPClient:
    ...
    async def __aenter__(self) -> "MCPClient":
        # Open HTTP stream to MCP server
        self._streams_context = streamablehttp_client(self.mcp_server_url)
        read_stream, write_stream, _ = await self._streams_context.__aenter__()

        # Create and enter MCP client session
        self._session_context = ClientSession(read_stream, write_stream)
        self.session = await self._session_context.__aenter__()

        # Initialize and print server capabilities
    ...
    async def __aexit__(self, exc_type, exc_val, exc_tb) -> None:
        # Properly tear down session and HTTP streams
    ...
    async def get_resources(self) -> list[Resource]:
        ...
        try:
            ...
            print(f"Server doesn't support list_resources: {exc}")
            return []
    ...
    async def get_resource(self, uri: AnyUrl) -> str:
        ...
        if isinstance(content, BlobResourceContents):
            return content.blob  # type: ignore[return-value]
    ...
    async def get_prompts(self) -> list[Prompt]:
        ...
        try:
            ...
            print(f"Server doesn't support list_prompts: {exc}")
            return []
